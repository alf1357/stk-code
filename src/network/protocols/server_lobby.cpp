...
void ServerLobby::changeTeam(Event* event)
{
    if (!ServerConfig::m_team_choosing ||
        !RaceManager::get()->teamEnabled() || ServerConfig::m_supertournament)
        return;
...
void ServerLobby::asynchronousUpdate()
{
    ...
    case WAITING_FOR_START_GAME:
    {
        if (ServerConfig::m_owner_less)
        {
            ...
            if ( (m_timeout.load() < (int64_t)StkTime::getMonoTimeMs() ||
                ...
                (int)players >= ServerConfig::m_min_start_game_players)) && (int)active_players >= 1)
            {
                resetPeersReady();
                startSelection();
                return;
    ...
    case WAIT_FOR_WORLD_LOADED:
    {
        ...
        if (ServerConfig::m_supertournament)
        {
            ...
            m_tournament_manager.GetCurrentResult(red_goals, blue_goals);
...
void ServerLobby::startSelection(const Event *event)
{
    ...
    if (ServerConfig::m_supertournament)
    {
        ServerConfig::m_fixed_lap_count = std::max(m_tournament_manager.GetAdditionalMinutesRounded(), 1);
    }
    ...
    switch (RaceManager::get()->getMinorMode())
    {
        ...
        case RaceManager::MINOR_MODE_SOCCER:
        {
            if (ServerConfig::m_fixed_lap_count >= 0)
            {
                m_default_vote->m_num_laps = ServerConfig::m_fixed_lap_count;
            } 
            ...
            m_default_vote->m_reverse = 0; // disable random distribution of items
            break;
        }
        default:
            assert(false);
...
void ServerLobby::updatePlayerList(bool update_when_reset_server)
{
    ...
    for (auto profile : all_profiles)
    {
        ...
        auto user_name = StringUtils::wideToUtf8(profile_name);
        ...
        if (m_player_queue_limit > 0)
        {
            // Add a symbol for the queue position
...
void ServerLobby::configPeersStartTime()
{
    ...
    *ns += *m_items_complete_state;
    ...
    World* w = World::getWorld();
    w->setPhase(WorldStatus::SERVER_READY_PHASE);
                                      ...
                                      std::shared_ptr<STKPeer> peer)
{
    ...
    else if (argv[0] == "game")
    {
        ...
        if (ServerConfig::m_supertournament)
        {
            if (m_tournament_manager.GameOpen())
            {
                msg = "There is still a game in progress. Play the additional time, or use /end to force terminating it.";
                sendStringToPeer(msg, peer);
                return;
            }
            else if (m_tournament_manager.GameDone(game))
            {
                if (argv.size() >= 3)
                {
            ...
            {
                m_tournament_manager.StartGame(game, length * 60);
            }
        }
    ...
    else if (argv[0] == "stop")
    {
        if (!isVIP(peer) && !isTrusted(peer) )
        {
         ...
         if (!w)
             return;
         SoccerWorld *sw = dynamic_cast<SoccerWorld*>(w);
         m_tournament_manager.StopGame(sw->getElapsedTime());
         sw->stop();
    ...
    else if (argv[0] == "go")
    {
        ...
        if (!w)
            return;
        SoccerWorld *sw = dynamic_cast<SoccerWorld*>(w);
        m_tournament_manager.ResumeGame(sw->getElapsedTime());
        sw->resume();
    ...
    else if (argv[0] == "init")
    {
        ...
        m_tournament_manager.SetCurrentResult(red, blue);
    ...
    else if (argv[0] == "end")
    {
        ...
        m_tournament_manager.ForceEndGame();
...
bool ServerLobby::isVIP(STKPeer* peer) const
{
    ...
    return !peer->getPlayerProfiles()[0]->isOfflineAccount() && m_vip_players.count(username);
}   // isVIP
//-----------------------------------------------------------------------------
...
bool ServerLobby::isTrusted(STKPeer * peer) const
{
    ...
    return !peer->getPlayerProfiles()[0]->isOfflineAccount() && (m_vip_players.count(username) || m_trusted_players.count(username));
}  // isTrusted
//-----------------------------------------------------------------------------
...
void ServerLobby::onTournamentGameEnded()
{
    ...
    if (w)
    {
        ...
        {
            std::string add_time_msg = m_tournament_manager.GetAdditionalTimeMessage();
